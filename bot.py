...
async def cmd_help(message: Message, state: FSMContext):
    ...
    help_text = (
        ...
        "3️⃣ Надішли дані одним повідомленням (опис, соцмережі, зображення — якщо потрібно).\n"
...
# 🟢 Обробка вибору для всіх категорій
...
async def finish_submission(user: types.User, state: FSMContext, photos: list):
    ...
    text = (
        f"📥 <b>Нова заявка від</b> <a href=\"tg://user?id={user.id}\">{user.first_name}</a>\n"
